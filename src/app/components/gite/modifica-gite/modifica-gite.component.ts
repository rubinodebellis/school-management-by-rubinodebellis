...
@Component({
  ...
  templateUrl: './modifica-gite.component.html',
  styleUrl: './modifica-gite.component.css',
})
export class ModificaGiteComponent implements OnInit,OnDestroy{
...
be considered as 2024 - 03 - 02`
  ...
  ngOnInit(){
    this.route.paramMap.subscribe((p:ParamMap)=>{
      this.id = +p.get('id')!
      this.u.idModifica.trips = +p.get('id')!
      this.u.modificaAttiva.trips = true
    })
  ...
  ngOnDestroy(): void {
    this.id = null
    this.u.idModifica.trips = null
    this.u.modificaAttiva.trips = false
  }
  ...
  modificaElemento(f:NgForm){
    ...
    this.u.datiUtente['trips'][this.id] = 
      {
        id:this.id,
        ...
        deleting:this.u.datiUtente['trips'][this.id]['deleting']
      }

    this.fb.salvataggio()
    this.u.updateHeader()
    f.reset()
    this.r.navigate(['/trips/'+this.u.datiUtente['trips'][this.id]['id']])
  }
}
