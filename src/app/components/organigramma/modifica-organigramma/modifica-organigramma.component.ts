...
@Component({
  selector: 'app-modifica-organigramma',
  standalone: true,
  imports: [MatIconModule,RouterModule,MatFormFieldModule,FormsModule,MatOptionModule,MatFormField,MatLabel,MatSelectModule,MatLabel,MatSelect,MatInputModule,MatButtonModule],
  templateUrl: './modifica-organigramma.component.html',
  styleUrl: './modifica-organigramma.component.css',
})
export class ModificaOrganigrammaComponent implements OnInit,OnDestroy{
...
be considered as 2024 - 03 - 02`
  ...
  ngOnInit(){
    this.route.paramMap.subscribe((p:ParamMap)=>{
      this.id = +p.get('id')!
      this.u.idModifica.flowchart = +p.get('id')!
      this.u.modificaAttiva.flowchart = true
    })
  }

  ngOnDestroy(): void {
    this.id = null
    this.u.idModifica.flowchart = null
    this.u.modificaAttiva.flowchart = false
  }
  ...
  modificaElemento(f:NgForm){
    ...
    this.u.datiUtente['flowchart'][this.id] = 
      {
        id:this.id,
        name:f.value.nome,
        ...
        roles:f.value.ruolo,
        deleting:this.u.datiUtente['flowchart'][this.id]['deleting']
      }
    this.fb.salvataggio()
    this.u.updateHeader()
    f.reset()
    this.r.navigate(['/flowchart/'+this.u.datiUtente['flowchart'][this.id]['id']+'/'+this.u.datiUtente['flowchart'][this.id]['name']+'/'+this.u.datiUtente['flowchart'][this.id]['surname']])
  }
}